...
class TopicAnalysisSkill(BaseSkill):
    ...
    def _generate_titles_batch(self, trends, brand_config):
        ...
        5. **【最高禁令 - 绝不套模板】**：大模型在批量生成时最容易犯的错误就是“发明一个爆款句式然后一直换主语套用”（比如“同样是XX，为什么XX能成为硬通货”连续用10遍）。**我将使用严格的去重算法审查你的输出。你生成的这 {expected_total} 个标题，句式结构、修辞手法、疑问句/反问句/陈述句的使用必须100%完全不同！绝不允许出现同一种语法结构的复读机行为！**
           - **【惩罚机制】**：如果检测到你输出了超过2个结构雷同的标题（即只替换了主语/宾语的填词游戏），系统将判定为恶意注水，**不仅会直接扣除本次计算的所有 Token 费用，还会将该模型的调用权限降级！请你必须进行严格的逐条自我审查！**
        6. **字段溯源约束**：为了关联热点跟踪，你必须在每条返回的 JSON 里附带 "source_topic" 字段，写入当时你参照的原热点完整名称。
        ...
        # 将温度从 0.7 提升到 0.85，强迫大模型在词汇和句式上做更多随机扩散，防止坍缩到同一个模板
        res = llm_utils.call_llm_json_array(prompt, model=config.TITLE_MODEL, temperature=0.85, max_retries=2)
