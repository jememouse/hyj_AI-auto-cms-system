...
class DeepWriteSkill(BaseSkill):
    ...
    def _build_prompt(self, dynamic_links, topic, category, category_id, brand_name, selected_city, rag_context, category_instruction, source_trend=""):
        ...
        0. **史诗级长文与结构化深度**: 全文必须是一篇 **2000字以上** 的深度支柱长文（Pillar Content）。**极度重要：每一个 H2 章节下方，绝对不能只有一两段话蜻蜓点水！** 必须强制将其拆解出 3-4 个细分的深入剖析点（可使用加粗小标题、深度段落或带数据的参数列表展开）。在保持口语易读的同时，必须用极其详尽的技术细节、工艺流程或逻辑推导把文章的“血肉”填满！
        1. **结构与强互动 HTML**: 
