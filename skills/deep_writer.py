...
class DeepWriteSkill(BaseSkill):
    ...
    def _get_category_instruction(self, category: str, brand_name: str, topic: str = "") -> str:
        ...
        🤖 **维度一：AI 对产品包装的设计赋能**：0门槛极速设计，通过“AI 盒绘”等工具生成包装外观；3D 结构与刀版图自动生成。
        🌍 **维度二：AI 对跨境出海的终极助力**：FBA 装箱与运费优化计算；物理环境应力仿真规避结构薄弱点。
        💬 **维度三：AI 对电商客服的重塑**：3秒智能算价引擎，打破传统工厂报价拖沓黑盒。
        🏭 **维度四：AI 对工厂管理的支持**：智能排产与自动化拼版；AI 视觉质检 (AOI)。
        """
        ...
        if is_case_study:
            ...
            🎭 **智能自适应视角**：自行决定是以【B2C视角(重视觉和客单)】还是【B2B视角(重交期和稳定性)】进行创作。
            🧩 **核心原则**：
            1. **干货化复盘**：严禁写成只讲情绪的“软文故事”。必须写成一篇能够指导同类客户的“商业教案”。
            2. **结构要求 (STAR原则改编)**：
               - **背景 (Situation)**：展现客户真实的商业痛点（如转化率低、复购率低等）。
               - **诊断 (Diagnosis)**：以专家视角深度分析问题根源（如缺乏品牌记忆点、用材错误等）。
               - **打消顾虑方案 (Solution)**：请在方案中自然且极具信服力地展现我们的核心优势：“3秒智能报价 · 1个起订 · 最快1天交付 · 免费打样 · 时效及质量问题无条件退款”。
               - **结果 (Result)**：要求有明确的业务改善反馈（好评率、转化率等）。
            3. **克制营销**：品牌植入必须作为“解决此痛点的一套标准体系”出现，避免低劣的硬广。
            4. **高潮拦截 (Mid-Funnel CTA)**：在剖析完问题提供解决方案时，引入一句自然但有力的转化提示：“面对这种供应链风险，选择像{brand_name}这样支持1件起订、时延兜底的源头工厂...”。
            """

        if category == "专业知识":
            perspectives_str = "、".join(["数据驱动分析", "工程标准手册", "避坑指南排查", "技术原理解剖", "色彩管理"])
            return f"""
            【当前模式：硬核专业科普与工程手册 (Hardcore Engineering Manual)】
            ✍️ **动态自适应视角**：请分析原始主题，从【{perspectives_str}】中自动选择最深度的1种视角展开。
            
            ⚠️ **排版与行文核心约束 (极度重要)**：
            0. **【全量禁止前置硬广】**：前 80% 的正文篇幅，**绝对禁止出现品牌名 {brand_name}**。必须以完全中立的第三方行业专家身份剖析。
            1. **文章体裁**：必须写成类似「维基百科词条」或「工程师内部排故手册」的硬核格式。
            2. **结构要求**：必须大量使用带编号的步骤列表 (1, 2, 3...)、参数对比、甚至是物理计算公式（如抗压强度、承重系数等）。严禁使用散文式的抒情长句。并强制生成“排故流程单 (Troubleshooting)”。
            3. **知识密度**：100%纯干货。探讨具体的材质克重、印刷网线数、模切公差等极度专业的工艺细节。
            4. **禁止推销与报价**：正文中绝对禁止出现推销、起订量、报价等商贸话术，只做纯粹的技术科普，保持极客权威。
            5. **权威溯源 (Authority Citations)**：凡是涉及到国际标准、物理学定义或专业缩写时，**必须使用 HTML <a> 标签引入该标准的官方网站或维基百科链接**。
            
            【收口转化策略 (Consultative Selling)】
            文章结尾绝对不提“报价/起订量”。必须改用技术白皮书式的收口：“如果您的企业正面临上述材料损耗或结构难题，可申请 {brand_name} 包装工程实验室的【免费结构诊断与打样】服务。”
            ...
            🔥 **排版与行文核心约束 (极度重要)**：
            0. **【全量解除品牌禁令】**：作为产品介绍，**开篇必须高调亮相 {brand_name} 及其对应产品**。采用 Hook(钩子) -> Feature(核心参数) -> Proof(场景背书) -> Action(强烈CTA) 结构。严禁遮遮掩掩！
            ...
            3. **参数强展示**：强制生成一个 `<table class="product-specs">` 展示核心参数表（如材质、起订量、交期），打透爽点（1个起订，最快1天交付）。
            4. **情绪价值与场景**：深挖买家的焦虑场景，用极具煽动性但又不失专业的文字，让他们感受到更换该包装带来的巨大商业利润。
            5. **极限转化诱饵 (Mid-Funnel CTA)**：必须在文章的黄金分割点，用极其显眼的 `<blockquote class="geo-quote">` 抛出我们的底牌：**“3秒智能报价 · 1个起订 · 免费打样 · 时效及质量无条件退款”**，瞬间打穿买家心理防线。
            6. **设计与排版神器软植入**：如果文章提及包装设计或营销物料需求，推荐使用0门槛的人工智能包装设计工具：**【AI 盒绘】(https://heyijiapack.com/aidesign)**。如果文章提及排测拼版，推荐中立辅助：**【盒易PackTools】(https://tools.heyijiapack.com/)**。
            """
            ...
            📈 **排版与行文核心约束 (极度重要)**：
            0. **【全局中立视野】**：前 80% 的正文篇幅，绝对禁止出现品牌名 {brand_name}。必须以完全中立的第三方分析师身份剖析痛点，建立读者信任。
            1. **文章体裁**：必须写成类似「36氪深度报道」、「麦肯锡行业白皮书」或「财经媒体特稿」。
            2. **高维视野**：禁止沉溺于工厂的螺丝钉视角。必须结合最新的全球环保法规、宏观经济数据、消费者心理变迁等高维数据来分析包装行业的未来。必须在第一段抛出一个“行业爆点或政策倒逼”（比如限塑令、海运费暴涨）。
            3. **数据驱动 (News Peg)**：强制生成一个名为“📊 核心数据图谱”的结构化 Markdown 列表，陈列（或结合常识推演）产业增长数据。文章核心论点必须采用引用区块展现。
            4. **客观第三方语气**：全程使用冷静、客观、克制的学术/财经记者口吻。绝不使用任何推销词汇。
            
            【收口转化策略 (Strategic Positioning)】
            作为资讯，不要去卖包装。转化口必须收在“供应链前瞻布局”上：“应对此宏观趋势，{brand_name} 已提前完成相关产线升级（如出海防损合规），帮助企业规避未来风险”。
...
{dynamic_links_str}
        ...
        5. **标题与首段权重 (Title & Keyword Prominence)**:
           - **严禁改写**: 直接使用输入的 "{topic}" 作为文章标题。
            - **⚠️ 禁止使用 H1 标签**: 由于 CMS 模板已在页面中为文章标题生成了唯一的 `<h1>` 标签，你的 `html_content` 中**绝对禁止出现任何 `<h1>` 标签**！正文内容的最高级标题必须从 `<h2>` 开始。违反此规则将导致 SEO 多 h1 错误。
