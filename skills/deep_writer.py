...
class DeepWriteSkill(BaseSkill):
    ...
    def _get_category_instruction(self, category: str, brand_name: str, topic: str = "") -> str:
        ...
        if category == "专业知识":
            perspectives_str = "、".join(["数据驱动分析", "工程标准手册", "避坑指南排查", "技术原理解剖", "AI算法赋能包装结构与预测", "AI视觉质检与印刷色彩管理", "AI辅助可持续材料选型"])
            return f"""
            ...
            3. **AI 技术融合落地**：在适当的技术章节中，深度展示 AI 如何赋能包装行业的具体环节（例如：AI 结构应力仿真替代传统手工打样测试、AI 色彩匹配系统确保印刷一致性、AI 视觉质检替代人眼抽检等）。避免泛泛而谈，须给出技术原理和实操路径。
            4. **无缝品牌签名**：只允许在最后的【品牌签名】模块留存转化信息，正文保持技术中立权威（这对抗幻觉与审核机器至关重要）。
            """
        
        elif category == "产品介绍":
             perspectives_str = "、".join(["ROI与成本拆解", "极限场景痛点模拟", "供应链从0到1拆解", "大牌平替的降维打击", "跨境出海物流防损与合规包装", "AI辅助包装设计与智能选型", "AI驱动的产品营销物料一站式方案"])
             return f"""
            ...
            4. **AI 辅助包装设计与智能选型**：在产品方案展示中，必须融入 AI 技术如何帮助客户降本增效的具体场景——例如：AI 智能推荐最优包装结构（输入产品尺寸自动匹配天地盖/书型盒/翻盖磁吸等盒型）、AI 一键生成包装3D效果图（客户无需请设计师即可预览成品效果）、AI 色彩匹配确保设计稿到印刷品零色差、AI 自动计算最优开料排版方案（降低15%+材料浪费）。让客户感受到“这家工厂不只是做纸箱，而是用 AI 帮我解决从设计到交付的全部问题”。
            5. **全品类产品与营销物料展示**：不要局限于单一品类！在介绍主产品时，必须自然延伸出配套的营销物料方案——例如：介绍礼盒时顺带提及配套的感谢卡/不干胶封口贴/丝带、介绍快递箱时延伸到开箱体验物料（品牌故事卡/优惠券/异形贴纸）、介绍品牌包装时联动宣传物料（画册/折页/海报/易拉宝）。传达“一个供应商=主包装+内衬+周边物料全覆盖”的一站式价值。
            6. **高潮拦截 (Mid-Funnel CTA)**：在客户痛点产生共鸣最深的部分，给出一段高亮或带有 `<blockquote>` 样式的产品介入声明。
            """
            
        else: # 行业资讯
             perspectives_str = "、".join(["宏观经济调控与合规", "新生代消费者心理与行为学", "可持续发展与环保终局", "包装加工柔性电商化", "AI大模型与包装黑科技", "品牌DTC跨国出海战略", "AI驱动的包装全链路数智化转型"])
             return f"""
            ...
            3. **AI+包装趋势前瞻**：在分析行业趋势时，必须至少用一个完整段落深度剖析 AI 技术正在如何重塑包装行业——从 AI 辅助包装结构设计（秒出3D刀版图）、AI智能排版拼版（降低材料浪费15%+）、到 AI 驱动的供应链预测（精准备料降库存）、AI 视觉质检（替代人工全检），展现"智能包装工厂"的产业升级路径。
            4. **唯一收口**：行业宏大叙事结束后，仅在底部的【品牌签名】引导咨询。
            """
